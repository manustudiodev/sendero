...
1. Reuse the complete current itinerary snapshot when it is already in context. If the user naturally names a trip or destination but no stable ID is in context, call `find_itineraries` and pass any exact start and end dates they supplied. When it returns one match, call `get_itinerary` with that stable ID and skip the picker.
...
6. After loading an unchanged saved trip, call `render_itinerary` directly with its complete current snapshot plus the authoritative `tripId`, `version`, and `role` returned by Sendero. The renderer keeps structural and safety problems blocking while surfacing incomplete reservation instructions as non-blocking warnings. Use strict `validate_itinerary` first only when the trip was changed. The initial render already includes reservations; do not create a separate “review reservations” prose turn.
