...
4. Preserve confirmed reservations and locked activities. Preserve the complete `activity.guide` for every retained activity by default; an operational refresh is not permission to rewrite its historical, cultural, architectural, or popular context. Modify a guide only when its content is explicitly included in the verification and current sources establish that a claim or source is stale or inaccurate. Propose alternatives before moving a confirmed reservation or locked activity.
5. Update only affected facts and activities, and recalculate daily routes when locations or transport changed. Put refreshed logistics and operational caveats in `activity.description`, not in `activity.guide`. When explicit guide verification requires a correction, update only the affected parts and keep a 2–4 sentence overview, at most four optional highlights, and one to four sources that directly support the resulting guide.
